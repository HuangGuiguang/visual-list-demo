...
export default {
    data() {
        return {
            listData: [],
            positions: [],
            preItemSize: 50, // 预计的每项大小
            screenHeight: 0,
            currentOffset: 0,
            start: 0,
            end: 0,
            bufferPercent: 0.5, // 即每个缓冲区只缓冲 0.5 * 最大可见列表项数个元素
        };
    },
    created() {
    ...
    updated() {
        this.$nextTick(() => {
            ...
            // 更新完缓存列表后，重新赋值偏移量
            console.log("update currentOffset in updated");
    ...
    methods: {
        ...
        scrollEvent(target) {
            ...
            this.end = this.start + this.visibleCount;
            console.log("update currentOffset in scrollEvent");
            this.currentOffset = this.getCurrentOffset()
        },
        // 初始化列表
